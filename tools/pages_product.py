...
def gallery_html(p):
    """The product page gallery: one main frame + one tile per image.

    Markup contract with js/gallery.js, the only thing that moves the picture:
      [data-product-gallery]  the gallery root (on .pdp-media)
      #product-main-image     the big frame, also [data-gallery-main]
      [data-gallery-thumb]    one <button type="button"> per image
      data-full-image         that image's FULL-size file, never the card crop
      data-srcset/data-sizes  that same image's own responsive pair, when it
                              has one to offer — it travels with src, it is
                              never reused from another image
    Every value is read from the product record, so 1 image and 9 images need
    no special case and a new product needs no code change. Tiles are buttons,
    so a tap cannot navigate, reload, or fire a link that wraps them; with JS
    off the cover still renders as plain HTML.
    """
    im = p["images"]
    slug = p["slug"]
    main = im["main"]
    ...
    # (full file, card crop, full w, full h, alt, card w, card h)
    items = [(main, im.get("card"), im.get("fullW"), im.get("fullH"), alt_main,
              im.get("cardW"), im.get("cardH"))]
    for g in im.get("gallery", []):
        items.append((g["file"], g.get("card"), g.get("w"), g.get("h"),
                      f'{alt_main}: preview {len(items)}', g.get("cardW"), g.get("cardH")))
    if not items: return "", ""
    first = items[0]
    ...
    # The frame's srcset is image #1's own pair. The module keeps that rule for
    # every image it shows, which is why a srcset can no longer hold the
    # display hostage when a thumbnail is clicked.
    fss, fsz = gal_pair(2, slug, first[1], first[0], first[5], first[2])
    srcset = f' srcset="{esc(fss)}" sizes="{esc(fsz)}"' if fss else ""
    main_fig = f"""<figure class="gal-main">
      <img id="product-main-image" data-gallery-main src="{asset_file(2, slug, first[0])}"{srcset} width="{first[2] or 1200}" height="{first[3] or 800}" alt="{esc(first[4])}" fetchpriority="high" decoding="async" data-pin-description="{esc(pin_d)}" data-pin-url="{esc(pin_page)}" data-pin-media="{esc(main_pin_media)}">
    ...
    if len(items) > 1:
        btns = []
        for i, (f, card, w, h, alt, card_w, card_h) in enumerate(items):
            th = card or f
            iss, isz = gal_pair(2, slug, card, f, card_w, w)
            pair = f' data-srcset="{esc(iss)}" data-sizes="{esc(isz)}"' if iss else ""
            active = ' active' if i == 0 else ""
            here = ' aria-current="true"' if i == 0 else ""
            btns.append(
                f'<button class="gal-thumb product-gallery-thumb{active}" type="button"'
                f' data-gallery-thumb data-full-image="{esc(asset_file(2, slug, f))}"{pair}'
                f' data-alt="{esc(alt)}" data-w="{w or 1200}" data-h="{h or 800}"'
                f' data-pin-media="{esc(asset_abs(slug, f))}"'
                f' aria-label="View product image {i+1}"{here}>'
                f'<img src="{asset_file(2, slug, th)}"'
                f' width="{card_w or 150}" height="{card_h or 100}"'
                f' alt="{esc(alt)} thumbnail" loading="lazy" decoding="async"></button>')
        thumbs = f'<div class="gal-thumbs" role="group" aria-label="Product image previews">{"".join(btns)}</div>'
    ...
    for p in PRODUCTS:
      ...
      <div class="rel-arts">{links}</div>
        ...
        html_out = head(p["seoTitle"], p["seoDesc"], absurl(f"products/{slug}/"), depth,
                        ...
                        ctx=page_ctx("product", p), gallery=True)
...
<main id="main">
  <div class="wrap">
    ...
    <article class="pdp">
      <div class="pdp-media" data-product-gallery>{gal}</div>
      <div class="pdp-info">
